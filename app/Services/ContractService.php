...
class ContractService
{
    public function getExtendContract($contractId = 0)
    {
        return Contract::select('extend')
            ->where(['id' => $contractId])
            ->first();
    }
    ...
    public function getContractByPeriod($period = 0)
    {
        return Contract::select('tbl_contract.*')
            ->join('tbl_period_detail', 'tbl_period_detail.id', '=', 'tbl_contract.period_id')
            ->where(['tbl_period_detail.period_id' => $period, 'tbl_period_detail.active' => STATUS_ACTIVE, 'tbl_contract.active' => STATUS_ACTIVE])
            ->get();
    }
    ...
    public function updateExtendContract($contractId = 0)
    {
        return Contract::where('id', $contractId)
            ->where('active', STATUS_ACTIVE)
            ->update(['extend' => STATUS_ACTIVE]);
    }
}
